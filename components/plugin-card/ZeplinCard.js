...
import ZeplinIcon from "../../public/zeplin-icon.svg";
...
const ZeplinCard = (data) => {
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [selectwpFavs, setSelectedWpFavs] = useState();
  const [unauthorized, setUnauthorized] = useState(false);
  const [successModel, setSuccessModel] = useState(false);
  const [loaderState, setLoaderState] = useState(false);
  const fullStars = Math.floor(data?.plugin?.rating / 20);
  const halfStars = Math.round((data?.plugin?.rating % 20) / 10);
            ...
            <div className="plugin-header">
              <div className="plugin-h-left">
                <div className="plugin-icon">
                  <Image alt="" src={ZeplinIcon} />
                </div>
              </div>
              <div className="plugin-h-left">
                <h4 className="card-title mb-1">{data?.plugin?.name}</h4>
  ...
  `)}{" "}
          ...
          <div className="card-footer text-center cursor-pointer">
            <a
              className="text-white plugin-link cursor-pointer"
              onClick={() => {
                router.push(`/single-plugin-page?slug=${data?.plugin?.slug}`);
              }}
            >
