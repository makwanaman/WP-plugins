...
const JiraCard = (data) => {
  ...
  const fullStars = Math.floor(data?.pluginRelated?.rating / 20);
  const halfStars = Math.round((data?.pluginRelated?.rating % 20) / 10);
        ...
        if (access_token) {
          let pluginStore = await axios.post(
            `${process.env.NEXT_PUBLIC_BASE_URL}/plugins/add-to-wpfav`,
            {
              ...
              plugin_id: data?.pluginRelated?.id,
            },
            {
              headers: {
          ...
          <div className="card-body">
            ...
            <div className="plugin-header">
              ...
              <div className="plugin-h-right ">
                <h4 className="card-title mb-1 my-text">
                  {data?.pluginRelated?.name}
                </h4>
  ...
  `)}{" "}
          ...
          <div className="card-footer text-center">
            <a
              ...
              onClick={() => {
                router.push(
                  `/single-plugin-page?slug=${data?.pluginRelated?.slug}`
                );
              }}
            >
